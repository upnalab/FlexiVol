...
namespace Voxon
{
	...
	public class lightbuffer : MonoBehaviour, IDrawable
	{
		...
		public bool DrawWithSpheres = false;
		[Range(0.0001f,0.005f)]
		public float SphereRadius = 0.001f;
		...
		void Start()
		{
			...
			if(resolution == 0)
			{
				Debug.Log("Render Texture resolution is 0; stopping light system");
				return;
			}
...
//				System.Array.Copy(poltex_data, poltex_buffer, voxel_count);

				

				if (DrawWithSpheres)
				{
					VXProcess.Runtime.DrawSphereBulkCnt(poltex_data, SphereRadius, voxel_count);
				}
				else
				{
					VXProcess.Runtime.DrawUntexturedMesh(poltex_data, voxel_count, null, 0, 0, 0xffffff);
				}
			}
		}
	}
