...
def parse_calibration_settings_file(filename):
    ...
    if 'c0' not in calibration_settings.keys():
...
def save_extrinsic_calibration_parameters(R0, T0, R1, T1, prefix = ''):
    ...
    camera1_rot_trans_filename = os.path.join('camera_parameters', prefix + 'rot_trans_c1.dat')
...
if __name__ == '__main__':
    ...
    save_frames_single_camera('c0') #save frames for camera0
    save_frames_single_camera('c1') #save frames for camera1


    """Step2. Obtain camera intrinsic matrices and save them"""
    #camera0 intrinsics
    images_prefix = os.path.join('frames', 'c0*')
    ...
    #camera1 intrinsics
    images_prefix = os.path.join('frames', 'c1*')
    ...
    save_frames_two_cams('c0', 'c1') #save simultaneous frames


    """Step4. Use paired calibration pattern frames to obtain camera0 to camera1 rotation and translation"""
    frames_prefix_c0 = os.path.join('frames_pair', 'c0*')
    frames_prefix_c1 = os.path.join('frames_pair', 'c1*')
    ...
    check_calibration('c0', camera0_data, 'c1', camera1_data, _zshift = 60.)
