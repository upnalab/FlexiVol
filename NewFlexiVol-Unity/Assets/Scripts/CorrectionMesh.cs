...
public class CorrectionMesh : MonoBehaviour
{
        ...
        if (meshFilter != null)
        {
            ...
            if (needsDeformation)
            {
                ...
                for (int i = 0; i < vertices.Length; i++)
                {
                    ...
                    float newHeight;
                    newHeight = provisionalName(globalVertex.x, globalVertex.y);
    ...
    private float applyDeformation(float height, float x)
    {
        ...
        while(currHeight >= -2)
        {
            ...
            float paraboleAmplitude = getAmplitudeOfParabole(currHeight);
            ...
            Vector2 centerPoint = new Vector2(0, paraboleAmplitude);
    ...
    float getAmplitudeOfParabole(float x)
    {
    ...
    float provisionalName(float x, float initial)
    {
